...
import type { StripePaymentResponse } from './StripeBase';
import { property, PropertyValues } from 'lit-element';

import bound from 'bind-decorator';

import { StripeBase } from './StripeBase';
import { mapProps } from './lib/dom';
...
export class StripePaymentRequest extends StripeBase {
   ...
   * @readonly
   */
  @property({ type: Boolean, attribute: 'can-make-payment', reflect: true, readOnly: true, notify: true })
  readonly canMakePayment: Await<ReturnType<stripe.paymentRequest.StripePaymentRequest['canMakePayment']>> = null;
   ...
   */
  private async initPaymentRequest() {
   ...
   */
  private async initPaymentRequestButton() {
   ...
   */
  private async initPaymentRequestListeners() {
    ...
    switch (this.generate) {
      ...
      case 'token': this.paymentRequest.on('source', this.onPaymentResponse); break;
      case 'source': this.paymentRequest.on('token', this.onPaymentResponse); break;
    }
  }
   ...
   */
  @bound private async complete(paymentResponse: StripePaymentRequestResponse, confirmationError?: stripe.Error) {
   ...
   */
  @bound private async onPaymentResponse(paymentResponse: StripePaymentRequestResponse): Promise<void> {
   ...
   */
  @bound private async confirmPaymentIntent(paymentResponse: StripePaymentRequestResponse) {
   ...
   */
  @bound private async confirmCardPayment(data?: stripe.ConfirmCardPaymentData, options?: stripe.ConfirmCardPaymentOptions): Promise<PaymentIntentResponse> {
  ...
  private parseDatasets(selector: 'stripe-display-item'|'stripe-shipping-option'): stripe.paymentRequest.DisplayItem[] {
    const elements = [...this.querySelectorAll(selector)] as (StripeDisplayItem|StripeShippingOption)[];
    return !elements.length ? [] : elements
      .map(({ dataset }: StripeDisplayItem|StripeShippingOption) => dataset)
      .map(mapProps({
        amount: (x: string) => parseInt(x),
        label: (x: string): string => x,
        pending: (x: boolean): boolean => x,
      }));
  }
}
