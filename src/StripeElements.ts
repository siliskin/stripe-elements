...
export class StripeElements extends StripeBase {
   ...
   * @deprecated
   */
  @property({ type: Boolean, attribute: 'is-complete', reflect: true, notify: true, readOnly: true })
  isComplete = false;
   ...
   */
  @stripeMethod public async createPaymentMethod(paymentMethodData: stripe.PaymentMethodData = this.getPaymentMethodData()): Promise<stripe.PaymentMethodResponse> {
   ...
   */
  @stripeMethod public async createSource(sourceData: stripe.SourceOptions = this.sourceData): Promise<stripe.SourceResponse> {
   ...
   */
  @stripeMethod public async createToken(tokenData: stripe.TokenOptions = this.tokenData): Promise<stripe.TokenResponse> {
  ...
  public validate(): boolean {
    ...
    if (empty && !error) this.setReadOnlyProperties({ error: this.createError('Your card number is empty.') });
    return isValid;
  }
  ...
  private getStripeElementsStyles(): StripeStyleInit {
    ...
    const getStyle = (prop: string) => this.getCSSCustomPropertyValue(prop, computedStyle) || undefined;
    const styleReducer = ({ base = {}, complete = {}, empty = {}, invalid = {} }, camelCase: string) => ({
  ...
  protected async initElement(): Promise<void> {
    ...
    const options: stripe.elements.ElementsOptions = { hideIcon, hidePostalCode, iconStyle, style, value };
