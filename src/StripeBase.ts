...
function isPaymentMethodResponse(response: StripePaymentResponse): response is stripe.PaymentMethodResponse {
  return 'paymentMethod' in response;
}

function isSourceResponse(response: StripePaymentResponse): response is stripe.SourceResponse {
  return 'source' in response;
}

function isTokenResponse(response: StripePaymentResponse): response is stripe.TokenResponse {
  return 'token' in response;
}

/* istanbul ignore next */
const removeAllMounts = (slotName: SlotName) => (host: Element) =>
  host.querySelectorAll(`[slot="${slotName}"][name="${slotName}"]`)
    .forEach(remove);

const slotTemplate = (slotName: SlotName) =>
 ...
 */
export class StripeBase extends LitNotify(ReadOnlyPropertiesMixin(LitElement)) {
   ...
   */
  @property({ type: Object, notify: true, readOnly: true, reflect: true, converter: errorConverter })
   ...
   * @deprecated
   */
  @property({ type: Boolean, attribute: 'stripe-ready', reflect: true, notify: true, readOnly: true, })
   ...
   */
  protected stripeMountId: string;
   ...
   */
  private fireError(error: AmbiguousError) {
   ...
   */
  protected getCSSCustomPropertyValue(propertyName: string, precomputedStyle?: CSSStyleDeclaration) {
   ...
   * @return
   */
  @bound protected async handleResponse(response: StripePaymentResponse): Promise<StripePaymentResponse> {
  ...
  private initShadowMountPoints(): void {
    ...
    while (host = (host.getRootNode() as ShadowRoot).host)
  ...
  private async postRepresentation(): Promise<void> {
    const onError = (error: Error) => this.setReadOnlyProperties({ error });
   ...
   */
  @bound private representationChanged(name: string) {
